...
def computeCost(X, y, theta, lambd):
  ...
  c=np.dot(theta_t,theta)
  c1=np.dot(c,lambd)
  m=np.shape(X)[0]
  b=np.matmul(X,theta_t)
  a=np.multiply(y,b)
  cost=np.sum(np.maximum(0,1-a))/m+c1
  return cost
...
def computeGradient(X, y, theta, lambd):
    ...
    grad = np.zeros(n)
    m, n = X.shape
    temp0=np.dot(X,theta)
    #print("shape of temp0")
    #print(np.shape(temp0))

    temp1 = (y*temp0 <= 1.00).astype(int)
    for n_col in range(n):
        grad[n_col] = 2.0*lambd*theta[n_col] - 1.0/m * np.sum(y * X[:, n_col] * temp1)
...
def gradientDescent(X, y, theta,lambd):
  ...
  (m,n)=X.shape
  theta= np.zeros(n)
  #let's use a reasonable starting point for theta 
